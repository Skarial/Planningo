...
import { computeDailyRestWarning, formatRestMinutes } from "../domain/daily-rest-warning.js";
...
export async function showGuidedMonth(forcedDate = null) {
  ...
  if (!guidedRestWarningByMonth.has(monthISO)) {
  ...
  async function resumeCurrentDayFromDB() {
    // Recalcule depuis la DB pour eviter un decalage apres sortie/retour de la vue.
    for (let d = 1; d <= daysInMonth; d++) {
      const date = new Date(year, monthIndex, d);
      const iso = toISODateLocal(date);
      const entry = await getPlanningEntry(iso);
  ...
  async function findPreviousWorkedDayEntry(currentDate) {
    ...
    for (let i = 0; i < 62; i++) {
      ...
      const entry = await getPlanningEntry(iso);
      if (!entry || !entry.serviceCode) continue;
      if (String(entry.serviceCode).toUpperCase() === "REPOS") continue;
  ...
  const restWarningNode = document.createElement("div");
  restWarningNode.className = "guided-rest-warning";
  restWarningNode.setAttribute("role", "alert");
  restWarningNode.setAttribute("aria-live", "assertive");
  restWarningNode.hidden = true;
  const restWarningTextNode = document.createElement("div");
  restWarningTextNode.className = "guided-rest-warning-text";
  restWarningNode.appendChild(restWarningTextNode);
  saisieContent.appendChild(restWarningNode);

  function hideRestWarning() {
    restWarningNode.hidden = true;
    restWarningTextNode.textContent = "";
    restWarningNode.removeAttribute("data-warning");
    clearTimeout(restWarningNode.__timer);
  }

  function showRestWarning(text) {
    ...
    restWarningTextNode.textContent = "";
    restWarningNode.setAttribute("data-warning", normalized);
  ...
  async function renderCompletedView() {
    ...
    const completeNodes = [title, subtitle, statsList, btnNextGuided];
    if (alarmSyncEnabled) {
      ...
      alarmLabel.textContent = "Synchroniser le réveil";
      ...
      completeNodes.push(btnSyncAlarm);
    }
    completeNodes.push(btnMonth);

    servicesContainer.append(...completeNodes);
    if (guidedRestWarningText) {
      showRestWarning(guidedRestWarningText);
    } else {
      hideRestWarning();
    }
  }
  ...
  async function renderDay() {
    ...
    while (currentDay <= daysInMonth) {
      ...
      const testIso = toISODateLocal(testDate);
      ...
      const existingEntry = await getPlanningEntry(testIso);
      const hasServiceCode =
        existingEntry &&
        typeof existingEntry.serviceCode === "string" &&
        existingEntry.serviceCode.trim() !== "";

      // En saisie guidée, on saute toujours les jours déjà saisis.
      if (hasServiceCode) {
        currentDay++;
        continue;
      }
    ...
    weekdayLabel.textContent = weekdayLong.charAt(0).toUpperCase() + weekdayLong.slice(1);
    ...
    if (guidedRestWarningText && guidedRestWarningText.trim()) {
      showRestWarning(guidedRestWarningText);
    }
  }
